...
import pygame
import sys
# NHỚ THÊM 'menu_bg_surface' VÀO DÒNG NÀY
...
class Menu:
    ...
    def draw(self):
        # --- SỬA ĐOẠN VẼ NỀN ---
        if menu_bg_surface:
            # Nếu có ảnh thì vẽ ảnh (tọa độ 0,0 góc trái trên)
            screen.blit(menu_bg_surface, (0, 0))
        else:
            # Nếu không có ảnh thì tô màu xanh như cũ
    ...
    def draw_sub_screen(self, title, msg):
        # Vẽ đè một lớp mờ hoặc tô màu nền lại cho màn hình con cho dễ đọc
        if menu_bg_surface:
             screen.blit(menu_bg_surface, (0,0))
        else:
             screen.fill(GRASS_LIGHT)
