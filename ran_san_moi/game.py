...
class Game:
    def __init__(self):
        ...
        self.all_high_scores = self.hs_manager.load() # Load toàn bộ dict
        self.high_score = 0 # Điểm cao của map hiện tại
    ...
    def load_map(self, map_name, is_gameplay=True):
        ...
        if is_gameplay:
            ...
            self.high_score = self.all_high_scores.get(map_name, 0)
    ...
    def update_high_score(self):
        """Cập nhật điểm cao vào bộ nhớ chung"""
        if self.score > self.high_score:
            self.high_score = self.score
            # Lưu vào dict tổng
            self.all_high_scores[self.gameplay_map_name] = self.score
    ...
    def save_current_game(self):
        # Lưu Highscore tổng xuống file
        self.hs_manager.save(self.all_high_scores)
    ...
    def load_saved_game(self):
        ...
        try:
            ...
            if not data.get("game_running_status", True): return False

            self.load_map(data.get("map", "Kinh điển"))
            
            body_data = data.get("snake_body", [])
            if body_data: self.snake.body = deque([Vector2(p[0], p[1]) for p in body_data])
            ...
            else: self.food.special_position = None
    ...
    def start_countdown(self):
        ...
        if countdown_sound: countdown_sound.play()
    ...
    def draw_elements(self, screen, is_paused):
        ...
        if not self.game_running: self.renderer.draw_game_over(self.score)
    ...
    def check_eat_food(self):
        ...
        if int(head.x) == int(self.food.position.x) and int(head.y) == int(self.food.position.y):
            self.score += 1
            self.update_high_score() # [GỌI HÀM CẬP NHẬT]
        ...
        if self.food.special_position:
            if int(head.x) == int(self.food.special_position.x) and int(head.y) == int(self.food.special_position.y):
                self.score += 3
                self.update_high_score() # [GỌI HÀM CẬP NHẬT]
                if eat_special_sound: eat_special_sound.play()
                elif eat_sound: eat_sound.play() 
                self.food.special_position = None
    ...
    def game_over(self):
        ...
        try:
            ...
            self.hs_manager.save(self.all_high_scores)
            print(f"----> (GAME OVER) ĐÃ LƯU KỶ LỤC.")
        except Exception as e:
    ...
    def reset_game(self):
        ...
        # Cập nhật lại high score từ bộ nhớ (đề phòng)
        self.high_score = self.all_high_scores.get(self.gameplay_map_name, 0)
